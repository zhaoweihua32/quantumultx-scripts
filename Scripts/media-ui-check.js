...
// 修复：安全获取环境参数
var opts = {
  policy: $environment && $environment.params ? $environment.params : ''
};
...
const message = {
  ...
  content: $environment && $environment.params ? $environment.params : ''
};
  ...
  try {
    ...
    $configuration.sendMessage(message).then(resolve => {
      if (resolve.error) {
        ...
        // 修复：即使没有策略信息也显示结果
        showResult($environment && $environment.params ? $environment.params : '未知节点');
      }
      if (resolve.ret) {
        let output = JSON.stringify(resolve.ret[message.content]) ? JSON.stringify(resolve.ret[message.content]).replace(/\"|\[|\]/g,"").replace(/\,/g," ➟ ") : ($environment && $environment.params ? $environment.params : '未知节点')
        showResult(output);
      }
    }, reject => {
      showResult($environment && $environment.params ? $environment.params : '未知节点');
      ...
      if (response.statusCode == 200) {
        ...
        $task.fetch(option1).then(response => {
          ...
          if (response.statusCode == 200) {
            ...
            resolve("discovery check failed")
          }
        }, reason => {
        ...
        result["Discovery"] = "<b>Discoveryᐩ: </b>获取Token失败 ❗️"
        resolve("discovery token failed")
      }
    }, reason => {
